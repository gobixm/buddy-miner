...
const Miner = class Miner {
    ...
    async mine() {
        ...
        let jobs = _.values(this._mineJobs).map(job => job.worker);
    ...
    async stop() {
        ...
        this._stop = true;
        _.values(this._mineJobs).forEach(job => clearTimeout(job.timeout));
        let jobs = _.values(this._mineJobs).map(job => job.worker);
        await Promise.all(jobs);
    ...
    async _mineRealm(realm, job) {
        ...
        let url = `https://${
            this._config.battleNetUrl
        }/wow/auction/data/${realm}?locale=${this._config.locale}&apikey=${
            this._config.battleNetApiKey
        }`;
        ...
        return new Promise(async resolve => {
            try {
                ...
                let lastProcessed = (await this._consul.getValueAsync(key))
                    .lastProcessed;
                lastProcessed = lastProcessed
                    ? new Date(lastProcessed)
                    : undefined;

                logger.info("checking last modified", {
                    lastProcessed: lastProcessed,
                ...
                if (!lastProcessed || lastModified > lastProcessed) {
                    ...
                    await this._consul.setValueAsync(key, {
                        lastProcessed: lastModified
                    });
                } else {
                    logger.info("auction is up to date");
                }
                if (this._stop) {
                    resolve();
                }
                ...
                job.timeout = setTimeout(
                    async () => await this._mineRealm(realm, job),
                    this._config.checkIntervalMs
                );
            } catch (e) {
                logger.error(e);
    ...
    async _processData(realm, url, snapTime) {
        ...
        try {
            ...
            return new Promise(resolve => {
                ...
                let stream = res.data.pipe(JSONStream.parse("auctions.*")).pipe(
                    es.map((data, cb) => {
                        data.realm = realm;
                        data.time = snapTime;
                        publisher
                ...
                stream.on("close", () => {
                    publisher
                        .flushAsync()
                        .then(() =>
                            logger.info("auction data processed", {
                                realm: realm
                            })
                        )
                        .then(() => resolve());
